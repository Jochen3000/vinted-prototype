...
function Checkout() {
  ...
  return (
    <div className="max-w-[960px] mx-auto px-4 lg:px-6 py-6">
      ...
      <h1 className="text-2xl font-bold text-stone-900 dark:text-stone-100 mt-3 mb-6">
        Checkout
      </h1>
      ...
      <div className="grid lg:grid-cols-[1fr_360px] gap-8 items-start">
              ...
              {shippingOptions.map((opt) => (
                <label
                  ...
                  className={`flex items-center gap-3 border rounded-md p-3 cursor-pointer ${
                    shipping === opt.id
                      ? "border-vinted-green bg-teal-50 dark:bg-teal-900/30"
            ...
            {payment === "card" && (
              <div className="mt-4 space-y-3">
                <input
                  ...
                  className="w-full h-11 border border-stone-300 dark:border-stone-600 rounded-md px-3 text-sm outline-none focus:border-vinted-green"
                />
                <div className="grid grid-cols-2 gap-3">
                  <input
                    placeholder="MM / YY"
                    className="h-11 border border-stone-300 dark:border-stone-600 rounded-md px-3 text-sm outline-none focus:border-vinted-green"
                  />
                  <input
                    placeholder="CVC"
                    className="h-11 border border-stone-300 dark:border-stone-600 rounded-md px-3 text-sm outline-none focus:border-vinted-green"
        ...
        {/* Right: order summary */}
        <aside className="lg:sticky lg:top-32 border border-stone-200 dark:border-stone-800 rounded-lg p-5 space-y-4">
          <h2 className="font-semibold text-stone-900 dark:text-stone-100">
            Order summary
          </h2>
          ...
          <div className="flex gap-3">
            <img
              src={item.images[0]}
              alt={item.title}
              className="w-16 h-20 object-cover rounded-md bg-stone-100 dark:bg-stone-800"
            />
            <div className="text-sm">
              <p className="font-medium text-stone-900 dark:text-stone-100 leading-tight">
                {item.title}
              </p>
              <p className="text-stone-500 dark:text-stone-400">
                {item.size} · {item.condition}
              </p>
              <p className="text-stone-500 dark:text-stone-400">{item.brand}</p>
            </div>
          </div>

          <div className="border-t border-stone-100 dark:border-stone-800 pt-3 space-y-2 text-sm">
          ...
          <p className="flex items-start gap-2 text-xs text-stone-500 dark:text-stone-400">
            <ShieldCheck
              size={28}
              className="text-vinted-green shrink-0 -mt-1"
            />
            Your purchase is covered by Buyer Protection. Get a full refund if
            your item doesn't arrive, is damaged or not as described.
          </p>

          <p className="text-[11px] text-stone-400 dark:text-stone-500 flex items-center gap-1 justify-center">
