  ...
  return (
    <div className="flex items-center justify-between py-2 border-b border-stone-100 text-sm">
      ...
      <span className="flex items-center gap-1 text-stone-900">
        {value}
        {info && <Info size={14} className="text-stone-400" />}
      </span>
    </div>
  );
}
...
function ItemDetail() {
  ...
  return (
    <div className="max-w-[1280px] mx-auto px-4 lg:px-6 py-6">
      {/* Breadcrumb */}
      <nav className="text-xs text-stone-500 mb-4 flex items-center gap-1 flex-wrap">
      ...
      <div className="grid lg:grid-cols-[1fr_400px] gap-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 content-start">
          {item.images.map((src, i) => (
            <div
              key={i}
              className={`overflow-hidden rounded-md bg-stone-100 ${
        ...
        <div className="lg:sticky lg:top-32 self-start space-y-4">
          <div className="border border-stone-200 rounded-lg p-5">
            <h1 className="text-xl font-bold text-stone-900">{item.title}</h1>
            ...
            <p className="text-sm text-stone-500 mt-4">{gbp(item.price)}</p>
            ...
            <p className="text-xs text-stone-400 mt-2">
              Item price and all applicable fees are converted to your
              currency. <span className="text-vinted-green">Learn more</span>.
            </p>
            ...
            <div className="mt-5 space-y-2">
              ...
              <button className="w-full h-11 rounded-md border border-vinted-green text-vinted-green font-medium hover:bg-teal-50">
                Make an offer
              </button>
              <button className="w-full h-11 rounded-md border border-stone-300 text-stone-800 font-medium hover:bg-stone-50">
                Message seller
              </button>
              <div className="flex gap-2 pt-1">
                <button className="flex-1 h-10 rounded-md border border-stone-300 flex items-center justify-center gap-2 text-sm text-stone-700 hover:bg-stone-50">
                  <Heart size={18} /> 14
                </button>
                <button className="flex-1 h-10 rounded-md border border-stone-300 flex items-center justify-center gap-2 text-sm text-stone-700 hover:bg-stone-50">
          ...
          {/* Buyer protection */}
          <div className="border border-stone-200 rounded-lg p-4 flex gap-3">
          ...
          <div className="border border-stone-200 rounded-lg p-4">
            ...
            <p className="flex items-center gap-1 text-xs text-stone-500 mt-2">
              <MapPin size={13} /> {seller.location}
            </p>
            <p className="text-xs text-stone-400 mt-1">{seller.lastSeen}</p>
          </div>
      ...
      <section className="mt-12">
        <h2 className="text-lg font-bold text-stone-900 mb-4">Similar items</h2>
