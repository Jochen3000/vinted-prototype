...
function OrderDetail() {
  ...
  return (
    <div className="max-w-[960px] mx-auto px-4 lg:px-6 py-6">
      {/* Confirmation banner */}
      <div className="border border-teal-200 bg-teal-50 rounded-lg p-5 flex items-start gap-3 mb-6">
      ...
      <div className="grid lg:grid-cols-[1fr_340px] gap-8 items-start">
        <div className="space-y-5">
          ...
          <section className="border border-stone-200 rounded-lg p-5">
            <h2 className="font-semibold text-stone-900 mb-4">Order status</h2>
                    ...
                    {i < steps.length - 1 && (
                      <span
                        className={`w-0.5 flex-grow mt-1 ${
                          s.done ? "bg-vinted-green" : "bg-stone-200"
                        }`}
                        style={{ minHeight: 24 }}
                      />
                    )}
                  ...
                  <div className="-mt-0.5">
                    <p
                      className={`text-sm font-medium ${
                        s.done ? "text-stone-900" : "text-stone-400"
                      }`}
                    >
                      {s.label}
                    </p>
          ...
          {/* Item */}
          <section className="border border-stone-200 rounded-lg p-5">
            <h2 className="font-semibold text-stone-900 mb-3">Your item</h2>
            <Link to={`/items/${item.id}`} className="flex gap-3 group">
              <img
                src={item.images[0]}
                alt={item.title}
                className="w-20 h-24 object-cover rounded-md bg-stone-100"
              />
              <div className="text-sm">
                <p className="font-medium text-stone-900 group-hover:text-vinted-green">
                  {item.title}
                </p>
          ...
          <section className="border border-stone-200 rounded-lg p-5">
            ...
            <div className="text-sm text-stone-700 leading-relaxed">
              <p className="flex items-center gap-1 text-stone-500 mb-2">
          ...
          <div className="flex flex-wrap gap-3">
            <button className="h-11 px-4 rounded-md border border-stone-300 text-sm font-medium text-stone-800 hover:bg-stone-50 flex items-center gap-2">
              <MessageCircle size={18} /> Contact seller
            </button>
            <button className="h-11 px-4 rounded-md border border-stone-300 text-sm font-medium text-stone-800 hover:bg-stone-50">
              I have an issue
            </button>
          </div>
        </div>
        ...
        <aside className="space-y-5">
          ...
          <section className="border border-stone-200 rounded-lg p-5">
            <h2 className="font-semibold text-stone-900 mb-3">Seller</h2>
